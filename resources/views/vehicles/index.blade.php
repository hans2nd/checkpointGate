...
<div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            @if(Auth::user()->hasPermission('vehicle.delete'))
            <button type="button" id="bulkDeleteBtn" onclick="doBulkDelete()" class="hidden items-center gap-2 px-5 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-bold rounded-xl shadow-sm transition-all hover:shadow-md">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                Hapus Terpilih (<span id="selectedCount">0</span>)
            </button>
            @endif
            @if(Auth::user()->hasPermission('vehicle.create'))
            <button type="button" onclick="document.getElementById('importModal').classList.remove('hidden')" class="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded-xl shadow-sm transition-all hover:shadow-md">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                Import Excel
            </button>
            @endif
            <a href="{{ route('vehicles.export', request()->query()) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-500 hover:bg-blue-600 text-white text-sm font-bold rounded-xl shadow-sm transition-all hover:shadow-md">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Export Excel
            </a>
            @if(Auth::user()->hasPermission('vehicle.create'))
            <a href="{{ route('vehicles.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-orange-500/30 transition-all hover:-translate-y-0.5">
    ...
    <div class="checkpoint-filter-card mb-6">
        <form method="GET" action="{{ route('vehicles.index') }}" class="flex flex-col lg:flex-row gap-4 items-center">
            <div class="flex-1 w-full relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari no polisi, driver, vendor..." class="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all shadow-sm">
            </div>
            <select name="vendor" class="border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white transition-all shadow-sm w-full lg:w-48">
            ...
            <div class="flex gap-2 w-full lg:w-auto">
                ...
                <a href="{{ route('vehicles.index') }}" class="flex-1 lg:flex-none px-6 py-3 bg-white hover:bg-gray-50 border border-gray-200 text-gray-600 text-sm font-bold rounded-xl transition-all text-center text-decoration-none shadow-sm hover:shadow-md">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr style="background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); color: white;" class="text-left border-b border-orange-600 shadow-sm">
                        @if(Auth::user()->hasPermission('vehicle.delete'))
                        <th class="px-4 py-3"><input type="checkbox" id="selectAll" onchange="toggleSelectAll()" class="rounded border-white/30 bg-white/20 text-orange-600 focus:ring-white"></th>
                        @endif
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider">No</th>
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider">No Polisi</th>
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider">Driver</th>
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider">Vendor</th>
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider">Tipe</th>
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider">Jenis Kendaraan</th>
                        <th class="px-4 py-3 font-semibold text-white text-xs uppercase tracking-wider text-center">Aksi</th>
                    </tr>
                </thead>
