...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="premium-form-card">
        <div class="form-header flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-white">Tambah Kendaraan Baru</h2>
            </div>
        </div>

        <form method="POST" action="{{ route('vehicles.store') }}" class="p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="form-group md:col-span-2">
                    ...
                    <input type="text" id="no_polisi" name="no_polisi" value="{{ old('no_polisi') }}" required placeholder="B 1234 XX"
                           class="premium-input uppercase">
                </div>

                <div class="form-group">
                    <label for="driver">Nama Driver <span class="text-orange-500">*</span></label>
                    <input type="text" id="driver" name="driver" value="{{ old('driver') }}" required placeholder="AKBAR"
                           class="premium-input uppercase">
                </div>

                <div class="form-group">
                    <label for="vendor">Vendor <span class="text-orange-500">*</span></label>
                    <input type="text" id="vendor" name="vendor" value="{{ old('vendor') }}" required placeholder="VITRANS"
                           class="premium-input uppercase">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="tipe" name="tipe" required class="premium-input bg-white">
                        <option value="EKSTERNAL" {{ old('tipe') == 'EKSTERNAL' ? 'selected' : '' }}>EKSTERNAL</option>
                ...
                <div class="form-group">
                    ...
                    <select id="jenis_kendaraan" name="jenis_kendaraan" required class="premium-input bg-white">
                        ...
                        @foreach($jenisKendaraanList as $jk)
                            <option value="{{ $jk }}" {{ old('jenis_kendaraan') == $jk ? 'selected' : '' }}>{{ $jk }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
