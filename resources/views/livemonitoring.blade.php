...
@section('content')
    ...
    <style>
        ...
        body.fullscreen-mode .monitoring-slider {
            flex: 1;
            display: flex;
            flex-direction: column;
            overflow: hidden;
        ...
        body.fullscreen-mode .slider-track {
            flex: 1;
        }
    </style>
    ...
    <div class="space-y-3" id="monitoring-container">
        ...
        <div class="monitoring-slider" id="monitoring-slider">
            <div class="slider-track" id="slider-track">
                ...
                {{-- ===== SLIDE 1: Activity Summary + Average Loading Time ===== --}}
                <div class="slider-slide active" data-slide="0">
                                ...
                                <table class="w-full text-xs">
                                    <thead>
                                        <tr class="border-b border-gray-100">
                                            ...
                                            <th class="text-center font-semibold text-gray-600 py-1.5 px-4">
                                    ...
                                    <tbody id="activity-summary-body" class="divide-y divide-gray-50">
                                        @foreach ($activitySummary as $row)
                                            <tr class="hover:bg-gray-50/50">
                                                <td class="py-2 pr-4 font-semibold text-gray-800 text-xs">
                                                    {{ $row['label'] }}</td>
                            ...
                            <div class="p-4 overflow-x-auto">
                                ...
                                <table class="w-full text-xs border-collapse">
                                    <thead>
                                        ...
                                        <tr>
                                            <th colspan="2"
                                                class="text-center font-semibold text-green-600 bg-green-50 py-0.5 text-[12px] border border-gray-200">
                                                ALT</th>
                                            <th colspan="2"
                                                class="text-center font-semibold text-green-600 bg-green-50 py-0.5 text-[12px] border border-gray-200">
                                                AUT</th>
                                            <th colspan="2"
                                                class="text-center font-semibold text-blue-600 bg-blue-50 py-0.5 text-[12px] border border-gray-200">
                                                ALT</th>
                                            <th colspan="2"
                                                class="text-center font-semibold text-blue-600 bg-blue-50 py-0.5 text-[12px] border border-gray-200">
                                                AUT</th>
                                            <th colspan="2"
                                                class="text-center font-semibold text-amber-600 bg-amber-50 py-0.5 text-[12px] border border-gray-200">
                                                ALT</th>
                                            <th colspan="2"
                                                class="text-center font-semibold text-amber-600 bg-amber-50 py-0.5 text-[12px] border border-gray-200">
                                                AUT</th>
                                        ...
                                        <tr>
                                            @foreach (['text-green-600 bg-green-50', 'text-green-600 bg-green-50', 'text-green-600 bg-green-50', 'text-green-600 bg-green-50', 'text-blue-600 bg-blue-50', 'text-blue-600 bg-blue-50', 'text-blue-600 bg-blue-50', 'text-blue-600 bg-blue-50', 'text-amber-600 bg-amber-50', 'text-amber-600 bg-amber-50', 'text-amber-600 bg-amber-50', 'text-amber-600 bg-amber-50'] as $idx => $cls)
                                                <th
                                                    class="text-center {{ $cls }} py-0.5 text-[12px] border border-gray-200 font-medium px-3">
                                    ...
                                    <tbody id="avg-times-body" class="divide-y divide-gray-50">
                                        @foreach ($avgTimes as $row)
                                            ...
                                            <tr class="hover:bg-gray-50/50">
                                                ...
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ $gc($row['DRY_ALT'] ?? null, $row['DRY_ALT_LMONTH'] ?? null) }}">
                                                    {{ $row['DRY_ALT'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ isset($row['DRY_ALT_LMONTH']) ? 'text-gray-500' : 'text-gray-300' }}">
                                                    {{ $row['DRY_ALT_LMONTH'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ $gc($row['DRY_AUT'] ?? null, $row['DRY_AUT_LMONTH'] ?? null) }}">
                                                    {{ $row['DRY_AUT'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ isset($row['DRY_AUT_LMONTH']) ? 'text-gray-500' : 'text-gray-300' }}">
                                                    {{ $row['DRY_AUT_LMONTH'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ $gc($row['FROZEN_ALT'] ?? null, $row['FROZEN_ALT_LMONTH'] ?? null) }}">
                                                    {{ $row['FROZEN_ALT'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ isset($row['FROZEN_ALT_LMONTH']) ? 'text-gray-500' : 'text-gray-300' }}">
                                                    {{ $row['FROZEN_ALT_LMONTH'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ $gc($row['FROZEN_AUT'] ?? null, $row['FROZEN_AUT_LMONTH'] ?? null) }}">
                                                    {{ $row['FROZEN_AUT'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ isset($row['FROZEN_AUT_LMONTH']) ? 'text-gray-500' : 'text-gray-300' }}">
                                                    {{ $row['FROZEN_AUT_LMONTH'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ $gc($row['CHILLED_ALT'] ?? null, $row['CHILLED_ALT_LMONTH'] ?? null) }}">
                                                    {{ $row['CHILLED_ALT'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ isset($row['CHILLED_ALT_LMONTH']) ? 'text-gray-500' : 'text-gray-300' }}">
                                                    {{ $row['CHILLED_ALT_LMONTH'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ $gc($row['CHILLED_AUT'] ?? null, $row['CHILLED_AUT_LMONTH'] ?? null) }}">
                                                    {{ $row['CHILLED_AUT'] ?? '—' }}</td>
                                                <td
                                                    class="py-1.5 px-3 text-center font-mono font-bold text-[14px] {{ isset($row['CHILLED_AUT_LMONTH']) ? 'text-gray-500' : 'text-gray-300' }}">
                    ...
                    </div> {{-- end slide-content 1 --}}
                </div> {{-- end slide 1 --}}

                {{-- ===== SLIDE 2: Frozen + Dry Gate Grid ===== --}}
                    ...
                    <p class="text-xs font-bold text-gray-700 flex items-center gap-1.5">
                        ...
                        <span class="text-gray-400 font-normal">(D-1 – D-11)</span>
                                ...
                                {{-- <button onclick="toggleGateFullscreen()" id="gate-fs-btn"
                                    ...
                                    <svg class="w-3.5 h-3.5" id="gate-fs-icon-expand" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
                                    </svg>
                                    <svg class="w-3.5 h-3.5 hidden" id="gate-fs-icon-compress" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 9V4.5M9 9H4.5M9 9L3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5l5.25 5.25" />
                                    </svg>
                            ...
                            <div class="p-3" id="gate-grid-body">
                                <div class="grid grid-cols-2 gap-3" id="gate-unified-grid">
                                    ...
                                    <div class="border-r border-gray-200 pr-3">
                                        <div
                                            class="text-center text-[14px] font-bold text-blue-600 mb-2 tracking-widest uppercase">
                                            ❄️ Frozen
                                        </div>
                                                ...
                                                <div id="gate-{{ $i }}" class="gate-card border-blue-300">
                                                    ...
                                                    {{ $cp->aktivitas === 'INBOUND' ? 'bg-orange-100 text-orange-700' : ($cp->aktivitas === 'OUTBOUND' ? 'bg-indigo-100 text-indigo-700' : '') }}">
                                                                ...
                                                                <p
                                                                    ...
                                                                    {{ $cp->no_polisi }}</p>
                                                                <p class="text-[12px] text-gray-800 leading-tight">
                                                                    {{ $cp->vendor }}</p>
                                    ...
                                    <div>
                                        <div
                                            class="text-center text-[14px] font-bold text-amber-600 mb-2 tracking-widest uppercase">
                                            🌡️ Dry
                                        </div>
                                        ...
                                        <div class="bg-white rounded-lg border border-gray-200 p-3">
                                            ...
                                            <div class="flex flex-wrap gap-4 text-xs text-gray-500">
                                                ...
                                                <span class="flex items-center gap-1.5">
                                                    <span
                                                        class="px-2 py-0.5 rounded-full text-[9px] font-bold bg-blue-100 text-blue-700">⏳
                                                        ON
                                                        LOADING</span>
                                            ...
                                            <div
                                                class="flex flex-wrap gap-4 text-xs text-gray-500 mt-2 pt-2 border-t border-gray-100">
                                                <span class="flex items-center gap-1.5"><span
                                                        class="w-3 h-3 rounded bg-blue-500"></span>
                                                    FROZEN (Gate F-1
                                                    – F-16)</span>
                                                <span class="flex items-center gap-1.5"><span
                                                        class="w-3 h-3 rounded bg-amber-500"></span>
                                                    DRY (Gate D-1 –
                                                    D-11)</span>
                                                <span class="flex items-center gap-1.5"><span
                                                ...
                                                <span class="flex items-center gap-1.5"><span
                                                    ...
                                                    menit</span>
                                                <span class="flex items-center gap-1.5"><span
                                                        class="w-3 h-3 rounded bg-red-500"></span>
                                                    Loading &gt; 60
                                                    menit</span>
                                            </div>
                                        </div>
            ...
            <div class="slider-indicators" id="slider-indicators">
                ...
                <button class="slider-dot active" data-goto="0" title="Activity Summary & Avg Loading Time"></button>
                <button class="slider-dot" data-goto="1" title="Gate Frozen & Dry"></button>
    ...
    <script>
        ...
        function fitMonitoringFullscreen() {
            if (gateIsFullscreen) return;

            const slider = document.getElementById('monitoring-slider');
            if (!slider) return;

            const isFs = document.body.classList.contains('fullscreen-mode');
            const headerEl = document.getElementById('monitoring-header');
            const indicatorsEl = document.getElementById('slider-indicators');
            const headerH = headerEl ? headerEl.offsetHeight : 0;
            const indicatorsH = indicatorsEl ? indicatorsEl.offsetHeight : 40;

            const availW = slider.clientWidth;
            let availH;

            if (isFs) {
                // Fullscreen: fill remaining viewport
                availH = window.innerHeight - headerH - indicatorsH - 24;
            } else {
                // Normal: fit within visible viewport area
                const sliderRect = slider.getBoundingClientRect();
                availH = window.innerHeight - sliderRect.top - indicatorsH - 16;
            }
            ...
            // Wait for layout recalc after resetting scale
            requestAnimationFrame(() => {
                document.querySelectorAll('.slide-content').forEach(content => {
                    ...
                    // Set parent slide height to scaled content height
                    const slide = content.closest('.slider-slide');
                    if (slide) {
                        slide.style.height = Math.ceil(naturalH * scale) + 'px';
                    }
                });
            });
        }
        ...
        const PROGRESS_TICK = 100; // progress bar update interval
        let currentSlide = 0;
        const totalSlides = 2;
        ...
        function goToSlide(index) {
            ...
            labels.forEach((l, i) => l.classList.toggle('active', parseInt(l.dataset.goto) === currentSlide));
            progressValue = 0;
        ...
        function startSliderAutoplay() {
            ...
            progressTimer = setInterval(() => {
                ...
                const pct = Math.min((progressValue / SLIDE_INTERVAL) * 100, 100);
        ...
        dots.forEach(dot => dot.addEventListener('click', () => {
            goToSlide(parseInt(dot.dataset.goto));
            startSliderAutoplay();
        }));
        labels.forEach(lbl => lbl.addEventListener('click', () => {
            goToSlide(parseInt(lbl.dataset.goto));
            startSliderAutoplay();
        }));
