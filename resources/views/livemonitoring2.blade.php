...
<div class="space-y-6" id="monitoring-container">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div class="flex items-center gap-3">
            <div class="p-2 bg-orange-100 rounded-lg">
                <svg class="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
            </div>
            <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                    ...
                    @if(!$isToday)
                        <a href="{{ route('livemonitoring2') }}" class="text-[10px] px-2 py-0.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded font-medium transition-colors">
                            Kembali Hari Ini
                        </a>
                    @endif
                </div>
        ...
        <div class="flex flex-col items-end gap-1">
            @if($isToday)
                <span id="refresh-indicator" class="flex items-center gap-1.5 text-sm text-orange-600 bg-orange-50 px-3 py-1.5 rounded-full font-bold shadow-sm">
                    <span class="w-2.5 h-2.5 bg-orange-500 rounded-full animate-pulse"></span>
                    LIVE REAL-TIME
                </span>
            @else
                <span class="flex items-center gap-1.5 text-sm text-gray-500 bg-gray-100 px-3 py-1.5 rounded-full font-bold shadow-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
                            @foreach($gateNumbers as $i)
                                @php
                                    ...
                                    $headerBg = $isFrozen ? 'bg-gradient-to-r from-blue-600 to-blue-400' : 'bg-gradient-to-r from-amber-500 to-orange-400';
                                    $typeLabel = $isFrozen ? 'FROZEN' : 'DRY';
                                    $displayGate = $isFrozen ? 'F-' . $i : 'D-' . ($i - 16);
                                @endphp
                                <div id="gate-{{ $i }}" class="rounded-2xl border-2 overflow-hidden transition-all duration-300 {{ $borderColor }} shadow-sm hover:shadow-md h-full flex flex-col">
                                    <div class="{{ $headerBg }} text-white text-center py-3 px-4 shadow-inner">
                                        <div class="flex justify-between items-center">
                                            <span class="text-xs font-bold tracking-widest opacity-90 border border-white/30 rounded px-1.5 pb-0.5">{{ $typeLabel }}</span>
                                    ...
                                    <div class="gate-body p-5 flex-1 flex flex-col items-center justify-center transition-all duration-500 min-h-[160px]"
                                        ...
                                        @if($cp)
                                            <div class="text-center w-full space-y-2">
                                                <p class="text-2xl font-black text-gray-800 tracking-wider truncate px-2">{{ $cp->no_polisi }}</p>
                                                <p class="text-sm font-semibold text-gray-500 truncate px-2 bg-white/50 inline-block rounded-md">{{ $cp->vendor }}</p>
                                                
                                                <div class="pt-3 border-t border-gray-900/10 mt-3 w-full flex flex-col items-center gap-2">
                                                    @if($cp->waktu_penyerahan_dokumen)
                                                        <span class="gate-status text-xs px-4 py-1.5 rounded-full font-bold bg-purple-100 text-purple-700 shadow-sm border border-purple-200"><span class="text-sm">✅</span> COMPLETED</span>
                                                    @elseif($cp->status === 'FINISH')
                                                        ...
                                                        <span class="text-sm font-black font-mono text-emerald-800 bg-white/60 px-3 py-1 rounded shadow-sm">{{ $cp->durasi ?? 'DONE' }}</span>
                                                    @elseif($cp->status === 'ON LOADING' && $cp->waktu_start)
                                                        ...
                                                        <span class="gate-timer text-2xl font-black font-mono text-blue-900 drop-shadow-sm" data-start="{{ $cp->waktu_start->toIso8601String() }}">00:00:00</span>
                                                    @elseif($cp->waktu_penerimaan_dokumen)
                                                        <span class="gate-status text-xs px-4 py-1.5 rounded-full font-bold bg-yellow-100 text-yellow-700 shadow-sm border border-yellow-200"><span class="text-sm">📋</span> ASSIGNED</span>
                                                        <p class="text-[10px] text-gray-500 mt-1 font-medium italic">Menunggu mulai loading...</p>
                                                    @endif
                                                </div>
                                            </div>
                                        @else
    ...
    {{-- Legend --}}
    <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm mt-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex-1">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2.5">Status Indicator</p>
                <div class="flex flex-wrap gap-x-6 gap-y-3 test-sm text-gray-600 font-medium">
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-yellow-400 animate-pulse"></span> Assign (Standby)</span>
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-blue-500 animate-pulse"></span> On Loading</span>
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-emerald-500"></span> Finish Loading</span>
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-purple-500"></span> Completed (Out)</span>
                </div>
            </div>
            <div class="flex-1 md:border-l md:border-gray-100 md:pl-6">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2.5">Durasi Waktu Loading</p>
                <div class="flex flex-wrap gap-x-6 gap-y-3 test-sm text-gray-600 font-medium">
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded bg-emerald-400 shadow-sm"></span> &lt; 30 Menit (Normal)</span>
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded bg-yellow-400 shadow-sm"></span> 30 - 60 Menit (Warning)</span>
                    <span class="flex items-center gap-2"><span class="w-3 h-3 rounded bg-red-400 shadow-sm"></span> &gt; 60 Menit (Delay)</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function getDurationColor(minutes) {
        ...
        return { bg: 'bg-emerald-100', text: 'text-emerald-800', border: 'border-emerald-400', bodyBg: '#ecfdf5' };
    }
    ...
    function getGateStatusHtml(gate) {
        if (gate.waktu_penyerahan) {
            ...
            let html = `<span class="gate-status text-xs px-4 py-1.5 rounded-full font-bold bg-emerald-100 text-emerald-700 shadow-sm border border-emerald-200"><span class="text-sm">🏁</span> FINISH LOADING</span>`;
            html += `<span class="text-sm font-black font-mono text-emerald-800 bg-white/60 px-3 py-1 rounded shadow-sm mt-2">${gate.durasi || 'DONE'}</span>`;
            return html;
        } else if (gate.status === 'ON LOADING' && gate.waktu_start) {
            let html = `<span class="gate-status text-xs px-4 py-1.5 rounded-full font-bold animate-pulse bg-blue-100 text-blue-700 shadow-sm border border-blue-200"><span class="text-sm">⏳</span> ON LOADING</span>`;
            const elapsed = formatDuration(gate.waktu_start, null);
            html += `<span class="gate-timer text-2xl font-black font-mono text-blue-900 drop-shadow-sm mt-2" data-start="${gate.waktu_start}">${elapsed}</span>`;
            return html;
    ...
    setInterval(function() {
        fetch('{{ route("livemonitoring2.data") }}?tanggal={{ $tanggalValue }}')
            ...
            .then(data => {
                ...
                const now = new Date();
                document.getElementById('last-update').textContent = 'Terakhir Sync: ' + now.toLocaleTimeString('id-ID');
            })
