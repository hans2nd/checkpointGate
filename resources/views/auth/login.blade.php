...
@section('content')
    {{-- Help Button --}}
    <a href="#" onclick="window.open(window.location.pathname.replace(/\/login.*/, '') + '/tutorial/', '_blank'); return false;" title="{{ __('Buku Panduan / Help') }}"
        ...
        <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 sm:p-8 shadow-2xl border border-white/10">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5" id="formEmail">
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-slate-400">{{ __('Ingat saya') }}</span>
                    </label>
                </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5 hidden" id="formEmployee">
                ...
                <div>
                    ...
                    <p class="mt-1.5 text-xs text-slate-500">{{ __('Login tanpa password menggunakan Employee ID') }}</p>
                </div>
