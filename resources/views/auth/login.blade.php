...
<div class="w-full max-w-md px-4">
    ...
    <div class="text-center mb-8">
        <div class="mb-4">
            <img src="{{ asset('images/logo-icon.png') }}" alt="Finna - PT. Pangan Lestari" class="h-20 w-20 mx-auto rounded-2xl object-cover shadow-lg shadow-orange-500/20">
        </div>
    ...
    <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 sm:p-8 shadow-2xl border border-white/10">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="remember" class="w-4 h-4 rounded border-white/20 bg-white/5 text-orange-500 focus:ring-orange-500 focus:ring-offset-0">
        ...
        <p class="mt-6 text-center text-sm text-slate-400">
            Belum punya akun?
            <a href="{{ route('register') }}" class="text-orange-400 hover:text-orange-300 font-medium transition-colors">Daftar</a>
        </p>
    </div>
</div>
@endsection
