        ...
        <form method="POST" action="{{ route('roles.update', $role) }}">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
                <div class="px-6 py-5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
                    <h2 class="text-lg font-bold text-gray-800">Edit Role: {{ $role->display_name }}</h2>
                    <p class="text-sm text-gray-500 mt-1">Ubah informasi dan atur permission role ini</p>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
                <div class="px-6 py-5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h2 class="text-lg font-bold text-gray-800">Permission</h2>
                                    ...
                                    <div class="flex flex-col gap-3">
                                        @foreach($perms as $permission)
                                            <label class="flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all hover:shadow-sm
                                                {{ in_array($permission->id, old('permissions', $rolePermissions)) ? 'border-indigo-300 bg-indigo-50/40 ring-1 ring-indigo-100' : 'border-gray-200 hover:border-indigo-200 hover:bg-gray-50/50' }}">
                                                <div class="pt-0.5">
