    ...
    <div class="max-w-xl mx-auto">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-800">{{ __('Edit Jenis Kendaraan') }}</h2>
                <p class="text-sm text-gray-500 mt-0.5">{{ __('Ubah nama jenis kendaraan.') }}</p>
            </div>

            <form method="POST" action="{{ route('vehicle-types.update', $vehicleType) }}" class="p-6">
                @csrf
                @method('PUT')
