    ...
    <div class="max-w-xl mx-auto space-y-6">
        ...
        <div class="premium-form-card">
            <div class="form-header flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold tracking-tight text-white">{{ __('Tambah Jenis Kendaraan') }}</h2>
                    <p class="text-slate-300 mt-1 text-sm font-medium">{{ __('Jenis kendaraan akan otomatis diubah menjadi huruf kapital.') }}</p>
                </div>
            </div>

            <form method="POST" action="{{ route('vehicle-types.store') }}" class="p-8">
                @csrf
