...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            ...
            <p class="text-sm text-gray-500 mt-0.5">{{ $checkpoint->no_polisi }} — {{ $checkpoint->tanggal->format('d/m/Y') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('checkpoints.update', $checkpoint) }}" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {{-- Tanggal --}}
                <div>
                    <label for="tanggal" class="block text-sm font-medium text-gray-700 mb-1.5">Tanggal <span class="text-red-400">*</span></label>
                    <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $checkpoint->tanggal->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="no_polisi" name="no_polisi" value="{{ old('no_polisi', $checkpoint->no_polisi) }}" required
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="vendor" name="vendor" value="{{ old('vendor', $checkpoint->vendor) }}" required
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all">
                </div>

                {{-- Driver --}}
                <div>
                    <label for="driver" class="block text-sm font-medium text-gray-700 mb-1.5">Driver <span class="text-red-400">*</span></label>
                    <input type="text" id="driver" name="driver" value="{{ old('driver', $checkpoint->driver) }}" required
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all">
                </div>
                ...
                {{-- Jenis Kendaraan --}}
                <div>
                    <label for="jenis_kendaraan" class="block text-sm font-medium text-gray-700 mb-1.5">Jenis Kendaraan</label>
                    <select id="jenis_kendaraan" name="jenis_kendaraan"
                        ...
                        @foreach(['FUSO', 'TRONTON', 'CONT-20FT', 'CONT-40FT', 'L300', 'CDE', 'CDE-LONG', 'CDD', 'CDD-LONG'] as $jk)
                            <option value="{{ $jk }}" {{ old('jenis_kendaraan', $checkpoint->jenis_kendaraan) == $jk ? 'selected' : '' }}>{{ $jk }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label for="waktu_penerimaan_dokumen" class="block text-sm font-medium text-gray-700 mb-1.5">Waktu Penerimaan Dokumen</label>
                ...
                <div>
                    <label for="waktu_penyerahan_dokumen" class="block text-sm font-medium text-gray-700 mb-1.5">Waktu Penyerahan Dokumen</label>
                ...
                <div>
                    ...
                    <input type="number" id="gate" name="gate" value="{{ old('gate', $checkpoint->gate) }}"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all">
                </div>

                {{-- Status --}}
                <div>
                    <label for="status" class="block text-sm font-medium text-gray-700 mb-1.5">Status <span class="text-red-400">*</span></label>
                    <select id="status" name="status" required
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white transition-all">
                        <option value="START" {{ old('status', $checkpoint->status) == 'START' ? 'selected' : '' }}>START</option>
                        <option value="FINISH" {{ old('status', $checkpoint->status) == 'FINISH' ? 'selected' : '' }}>FINISH</option>
                    </select>
                </div>

                {{-- Waktu Start --}}
                <div>
                    <label for="waktu_start" class="block text-sm font-medium text-gray-700 mb-1.5">Waktu Start</label>
                ...
                <div>
                    ...
                    <input type="text" id="durasi" name="durasi" value="{{ old('durasi', $checkpoint->durasi) }}"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all">
                </div>
            </div>
