        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @if (Auth::user()->hasPermission('checkpoint.delete'))
                    <form method="POST" action="{{ route('checkpoints.destroy', $checkpoint) }}"
                        onsubmit="return confirm('Yakin hapus data ini?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="inline-flex items-center gap-1.5 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 text-sm font-medium rounded-lg transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {{-- Each detail item --}}
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Tanggal') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">{{ $checkpoint->tanggal->format('d F Y') }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('No Polisi') }}</p>
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Tipe') }}</p>
                        <p class="text-sm mt-1">
                            <span
                                class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $checkpoint->tipe === 'INTERNAL' ? 'bg-purple-50 text-purple-700' : 'bg-sky-50 text-sky-700' }}">
                                {{ $checkpoint->tipe }}
                            </span>
                        </p>
                    </div>
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Jenis Kendaraan') }}
                        </p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">{{ $checkpoint->jenis_kendaraan ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Jenis Barang') }}</p>
                        <p class="text-sm mt-1">
                            <span
                                class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $checkpoint->jenis_barang === 'FROZEN' ? 'bg-cyan-50 text-cyan-700' : 'bg-orange-50 text-orange-700' }}">
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Waiting Time') }}</p>
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">
                            {{ __('Waktu Penerimaan Dokumen') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">
                            {{ $checkpoint->waktu_penerimaan_dokumen?->format('d/m/Y H:i:s') ?? '-' }}
                        </p>
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">
                            {{ __('Waktu Penyerahan Dokumen') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">
                            {{ $checkpoint->waktu_penyerahan_dokumen?->format('d/m/Y H:i:s') ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Durasi Dokumen') }}
                        </p>
                        <p class="text-sm text-gray-900 mt-1 font-mono font-medium">
                            @if ($checkpoint->waktu_penerimaan_dokumen && $checkpoint->waktu_penyerahan_dokumen)
                                @php
                                    $diff = $checkpoint->waktu_penerimaan_dokumen->diff(
                                        $checkpoint->waktu_penyerahan_dokumen,
                                    );
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Waktu Start') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">
                            {{ $checkpoint->waktu_start?->format('d/m/Y H:i:s') ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Waktu End') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">
                            {{ $checkpoint->waktu_end?->format('d/m/Y H:i:s') ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">
                            {{ __('Durasi Loading/Unloading') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-mono font-medium">{{ $checkpoint->durasi ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Dibuat Oleh') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">{{ $checkpoint->createdByUser?->name ?? '-' }}
                        </p>
                    </div>
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Cancel Oleh') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">{{ $checkpoint->canceledByUser?->name ?? '-' }}
                        </p>
                    </div>
                    ...
                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Dibuat') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">
                            {{ $checkpoint->created_at->format('d/m/Y H:i:s') }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ __('Diperbarui') }}</p>
                        <p class="text-sm text-gray-900 mt-1 font-medium">
                            {{ $checkpoint->updated_at->format('d/m/Y H:i:s') }}</p>
                    </div>
                </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-100">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <p class="text-xs font-medium text-gray-400 uppercase tracking-wider">
                                    {{ __('No. Surat Jalan') }}</p>
                                <p class="text-sm text-gray-900 mt-1 font-medium">{{ $checkpoint->no_surat_jalan ?? '-' }}
                                </p>
                            </div>
                            <div>
