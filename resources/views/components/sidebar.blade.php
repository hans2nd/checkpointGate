{{-- Sidebar Navigation --}}
<aside id="sidebar"
       class="sidebar-expanded bg-slate-900 text-white flex flex-col shrink-0 transition-all duration-300 ease-in-out
    ...
    <div class="px-4 py-4 border-b border-slate-700/50 flex items-center justify-between">
        ...
        <button onclick="closeSidebar()" class="lg:hidden p-1 text-slate-400 hover:text-white rounded-lg hover:bg-slate-700 transition-colors">
    ...
    <nav class="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        ...
        <div class="pt-3">
            <p class="sidebar-label px-3 text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Master Data</p>
        ...
        <div class="pt-3">
            <p class="sidebar-label px-3 text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Akun</p>

            {{-- Profile --}}
            <a href="{{ route('dashboard') }}"
               class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition-all duration-200"
               title="Profil">
    ...
    <div class="hidden lg:block px-2 py-2 border-t border-slate-700/50">
        <button onclick="toggleSidebarCollapse()" class="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-all" title="Toggle sidebar">
            <svg id="collapse-icon" class="w-5 h-5 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/></svg>
    ...
    <div class="px-3 py-3 border-t border-slate-700/50">
        <div class="flex items-center gap-3 overflow-hidden">
            <div class="w-8 h-8 bg-slate-700 rounded-full flex items-center justify-center text-xs font-bold text-orange-400 shrink-0">
