...
<aside id="sidebar"
    ...
    <nav class="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        ...
        {{-- Live Monitoring 2 --}}
        @if(Auth::user()->hasPermission('livemonitoring2.view'))
        <a href="{{ route('livemonitoring2') }}"
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200
                  {{ request()->routeIs('livemonitoring2') ? 'bg-orange-500/20 text-orange-400' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}"
            title="Live Monitoring v2 (Slider)">
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
