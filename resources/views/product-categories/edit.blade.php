...
@section('title', __('Edit Kategori Produk'))

@section('content')
<div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100">
    <div class="p-6 border-b border-gray-100">
        <h2 class="text-lg font-bold text-gray-900">{{ __('Edit Kategori Produk') }}</h2>
    </div>
        ...
        <form method="POST" action="{{ route('product-categories.update', $productCategory) }}">
            ...
            <div class="mb-5">
                ...
                <textarea id="description" name="description" rows="3"
                    ...
                    placeholder="Deskripsi kategori...">{{ old('description', $productCategory->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
