    ...
    <div class="space-y-5">
        ...
        <div class="report-filter-card">
            <div class="filter-header">
                ...
                <h3>{{ __('Filter') }} &amp;
                    {{ __('Search (License Plate, Vendor, Delivery Note, PO, Note)') }}<span>{{ __('Laporan data checkpoint berdasarkan periode dan filter') }}</span>
                </h3>
            </div>
            <form method="GET" action="{{ route('report.index') }}"
                ...
                <div class="filter-actions">
                    ...
                    @if (Auth::user()->hasPermission('report.export'))
                        <a href="{{ route('report.export', request()->query()) }}" class="btn-export"
        ...
        <div class="active-filters-bar {{ $hasActiveFilter ? 'has-filters' : '' }}">
            ...
            <span
                class="text-amber-700 font-bold text-xs uppercase tracking-wider">{{ __('Difilter Berdasarkan') }}:</span>
            <span class="text-amber-600 text-xs">
                {{ Carbon\Carbon::parse($startDate)->format('d/m/Y') }} —
                {{ Carbon\Carbon::parse($endDate)->format('d/m/Y') }}
            </span>
            @if ($aktivitas)
                <span class="tag">{{ __('Aktivitas') }}: {{ $aktivitas }}</span>
            @endif
            @if ($jenisBarang)
                <span class="tag">{{ __('Jenis Barang') }}: {{ $jenisBarang }}</span>
            @endif
            @if ($status)
                <span class="tag">{{ __('Status') }}: {{ $status }}</span>
            @endif
            @if ($search)
        ...
        <div class="report-summary-grid">
            ...
            <div class="summary-card">
                ...
                <div class="card-info">
                    <div class="card-value font-mono" style="color: #7c3aed; font-size: 1.1rem;">{{ $avgDurasi ?? '-' }}
                    </div>
                        ...
                        @forelse($checkpoints as $index => $cp)
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-3 py-2.5 text-gray-600 text-xs whitespace-nowrap">
                                    {{ $cp->type_of_load ?: '—' }}</td>
                                <td class="px-3 py-2.5 text-gray-600 text-xs whitespace-nowrap">
                                    {{ optional($cp->productCategory)->name ?: '—' }}</td>
                                <td class="px-3 py-2.5 text-gray-600 text-xs whitespace-nowrap">
                                    {{ $cp->shipping_type ?: '—' }}</td>
                                <td class="px-3 py-2.5 text-gray-600 text-xs whitespace-nowrap">
                                    @if ($cp->is_cross_dock)
                                        <span
                                            class="inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-green-50 text-green-700">TRUE</span>
                                    @else
                                        <span
                                            class="inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-gray-50 text-gray-700">FALSE</span>
                                    @endif
                                </td>
                                <td class="px-3 py-2.5 text-gray-600 text-xs whitespace-nowrap">
                                    @if ($cp->is_generated_cross_dock)
                                        <span
                                            class="inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-purple-50 text-purple-700">TRUE</span>
                                    @else
                                        <span
                                            class="inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-gray-50 text-gray-700">FALSE</span>
                                    @endif
